...
def test_pipeline_writes_parquet_to_local_fs(tmp_path: Path):
    ...
    kafka_connector_jar = os.path.join(root_folder, "jars", "flink-connector-kafka-4.0.1-2.0.jar")
    ...
    hadoop_runtime = os.path.join(root_folder, "jars", "hadoop-client-runtime-3.3.6.jar")
    ...
    raw_events = [
        json.dumps({"timestamp": 0, "wiki": "enwiki", "type": "edit", "user": "u1",
                    "bot": False, "title": "t1", "namespace": 0}),
        json.dumps({"timestamp": 1, "wiki": "dewiki", "type": "new", "user": "u2",
                    "bot": True, "title": "t2", "namespace": 1}),
    ]

    raw_stream = env.from_collection(raw_events, type_info=Types.STRING()) \
        .assign_timestamps_and_watermarks(WatermarkStrategy.no_watermarks())

    curated_stream = (
        raw_stream
        .map(parse_curated_row, output_type=Types.ROW([
            Types.INT(), Types.STRING(), Types.STRING(), Types.STRING(),
            Types.BOOLEAN(), Types.STRING(), Types.INT(), Types.STRING(), Types.STRING()
        ]))
    ...
    assert len(df) > 0
    assert {'event_ts', 'wiki', 'type', 'user_name', 'bot', 'title', 'namespace_id'}.issubset(df.columns)