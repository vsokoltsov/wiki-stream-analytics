...
def create_ingestion_pipeline(p: beam.Pipeline, args: Dict[str, Any]):
    ...
    events = (
        p
        | "ReadWithAttrs"
        >> ReadFromPubSub(subscription=args["subscription"], with_attributes=True)
    ...
    routed = ready | "AddPartitionInfo" >> beam.Map(
        add_partition_info, table_id=args["table_id"]
    )
    _ = routed | "LoadToBigQuery" >> beam.ParDo(
        LoadFolderToBQDoFn(project_id=args["project"])
    )
    return p
