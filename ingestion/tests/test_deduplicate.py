...
def test_key_for_dedup_includes_bucket_object_and_generation():
    e = {
        "bucket": "my-bucket",
...
def test_key_for_dedup_missing_generation_uses_empty_string():
    e = {"bucket": "b", "object": "path/to/obj"}
    assert key_for_dedup(e) == "b|path/to/obj|"


def test_key_for_dedup_different_objects_different_keys():
    e1 = {"bucket": "b", "object": "path/a", "generation": "1"}
    e2 = {"bucket": "b", "object": "path/b", "generation": "1"}
    assert key_for_dedup(e1) != key_for_dedup(e2)


def test_key_for_dedup_same_object_same_generation_same_key():
    e1 = {"bucket": "b", "object": "path/a", "generation": "1"}
    e2 = {"bucket": "b", "object": "path/a", "generation": "1"}
    assert key_for_dedup(e1) == key_for_dedup(e2)


def test_key_for_dedup_same_object_different_generation_different_key():
    e1 = {"bucket": "b", "object": "path/a", "generation": "1"}
...
def test_key_for_dedup_raises_keyerror_when_required_missing(missing):
...
def test_deduplicate_dofn_first_occurrence_yields_event():
...
def test_deduplicate_dofn_duplicate_key_yields_only_once():
...
def test_deduplicate_dofn_different_keys_both_yielded():
...
def test_deduplicate_dofn_three_events_two_keys_yields_two():
